...
# don't activate env if it already is (avoids long conda 'solving environment' time)
...
# if 'dry' is specified as target, any bumpversion call will not apply changes
...
help:
	...
	@echo "  converage-table   to display tests coverage analysis table."
...
install:
	...
	@-bash -c '$(CONDA_CMD) python setup.py develop'
	@echo "\nStart service with \`make start'"

.PHONY: install-dev
install-dev:
	@echo "Installing development requirements for tests and docs ..."
	@-bash -c '$(CONDA_CMD) pip install -r requirements_dev.txt'

.PHONY: start
start: conda-check
	@echo "Starting application ..."
	@-bash -c '$(CONDA_CMD) pserve development.ini &'
...
test: conda-check
	...
	@bash -c '$(CONDA_CMD) pytest -v -m "not slow and not online" tests/'

.PHONY: test-unit
test-unit: conda-check
	@echo "Running only unit tests ..."
	@bash -c '$(CONDA_CMD) pytest -v -m "not mongo and not slow and not online" tests/'

.PHONY: test-all
test-all: conda-check
	@echo "Running all tests (including slow and online tests) ..."
	@bash -c '$(CONDA_CMD) pytest -v tests/'
...
.coverage:
	...
	@bash -c '$(CONDA_CMD) pytest --cov tests/'
...
coverage-html: .coverage
	@bash -c '$(CONDA_CMD) coverage html -d coverage'
	@-echo "Coverage report: open file://$(CURDIR)/coverage/index.html"
...
docs: conda-check
	...
	@echo "open your browser: open file://$(CURDIR)/docs/build/html/index.html"
